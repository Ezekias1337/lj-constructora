...
// Types
import PaginationProps from './dependents/types-and-interfaces/PaginationProps'
// CSS
import './dependents/css/pagination.scss'
    ...
    return (
      <nav aria-label="Page navigation">
        <ul className="pagination">
          <li className={`${props.currentPage - 1 > 0 ? '' : 'disabled-page-item '}page-item`}>
            <a
              className={`${props.currentPage - 1 > 0 ? '' : 'disabled '}page-link`}
              onClick={() => parseReversePagination(props.currentPage, props.setCurrentPage)}
              aria-label="Previous"
            >
          ...
          <li
            className={`${props.currentPage < numberOfPages ? '' : 'disabled-page-item '}page-item`}
          >
            <a
              className={`${props.currentPage < numberOfPages ? '' : 'disabled '}page-link`}
              onClick={() =>
                parseForwardPagination(props.currentPage, props.setCurrentPage, numberOfPages)
              }
              aria-label="Next"
            >
